...
- **`/standup`'s filter buttons still say "Active", not "Open".** Spec #201's vocabulary rule
  (#208) replaced "active" with "open" in every user-facing string *except* these — the one-row
  filter keyboard and its `🔄 Active (N)` heading are copied verbatim from Devie, counts included,
