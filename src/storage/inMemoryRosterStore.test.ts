...
describe("InMemoryRosterStore", () => {
  it("upsert creates a new entry", async () => {
    ...
    await store.upsert({ username: "alice", role: "Intern", cohortId: "cohort-5" }, "bob");

    const all = await store.listAll();
    expect(all).toHaveLength(1);
    expect(all[0]).toMatchObject({ username: "alice", role: "Intern", cohortId: "cohort-5" });
  });

  it("upsert on an existing (cohortId, username) updates in place rather than duplicating", async () => {
    const store = new InMemoryRosterStore();
    await store.upsert({ username: "alice", role: "Intern", cohortId: "cohort-5" }, "bob");
    await store.upsert({ username: "alice", role: "HigherUp", cohortId: "cohort-5" }, "bob");

    const all = await store.listAll();
    expect(all).toHaveLength(1);
    expect(all[0]).toMatchObject({ username: "alice", role: "HigherUp", cohortId: "cohort-5" });
  });
  ...
  it("matches usernames case-insensitively and with a leading @ stripped, for both upsert and remove", async () => {
    const store = new InMemoryRosterStore();
    await store.upsert({ username: "@Alice", role: "Intern", cohortId: "cohort-5" }, "bob");
    await store.upsert({ username: "ALICE", role: "HigherUp", cohortId: "cohort-5" }, "carol");
