# PRD: DevCon PH Cohort 5 Task Bot (v1, original)

> **Archived.** This is the original v1 product spec, preserved exactly as written before any
> later redesign — kept as the historical record of what the bot looked like at launch. It does
> **not** describe the bot's current behavior. For that, see [`PRD.md`](../PRD.md), the current
> spec. `CONTEXT.md` and [`docs/adr/`](./adr/) cover the decisions that took the product from here
> to there — most significantly [ADR-0009](./adr/0009-devie-parity-command-redesign.md) (direct
> one-line commands, free-set statuses) and
> [ADR-0013](./adr/0013-remove-access-control-for-devie-parity.md) (all roles and permission
> checks removed).
